...
-- Populate existing vehicle slugs with unique handling
UPDATE vehicles 
SET slug = generate_slug(brand || '-' || name || '-' || year::text)
WHERE slug IS NULL;

-- Populate existing mod slugs with unique handling
UPDATE mods 
SET slug = generate_slug(title || '-' || id::text)
WHERE slug IS NULL;

-- Populate existing trail slugs with unique handling
UPDATE trails
SET slug = generate_slug(name || '-' || location)
WHERE slug IS NULL;
