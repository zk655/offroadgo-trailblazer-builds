-- Add slug columns to tables for SEO-friendly URLs
ALTER TABLE vehicles ADD COLUMN slug text;
ALTER TABLE mods ADD COLUMN slug text;
ALTER TABLE trails ADD COLUMN slug text;

-- Function to generate slug from text
CREATE OR REPLACE FUNCTION generate_slug(input_text text)
RETURNS text AS $$
BEGIN
  RETURN lower(
    trim(
      regexp_replace(
        regexp_replace(input_text, '[^a-zA-Z0-9\s-]', '', 'g'),
        '\s+', '-', 'g'
      ),
      '-'
    )
  );
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- Populate slugs with row number to ensure uniqueness
WITH numbered_vehicles AS (
  SELECT id, 
         generate_slug(brand || '-' || name || '-' || year::text) || 
         CASE 
           WHEN ROW_NUMBER() OVER (PARTITION BY generate_slug(brand || '-' || name || '-' || year::text) ORDER BY id) > 1 
           THEN '-' || ROW_NUMBER() OVER (PARTITION BY generate_slug(brand || '-' || name || '-' || year::text) ORDER BY id)::text
           ELSE ''
         END as unique_slug
  FROM vehicles 
  WHERE slug IS NULL
)
UPDATE vehicles 
SET slug = numbered_vehicles.unique_slug
FROM numbered_vehicles
WHERE vehicles.id = numbered_vehicles.id;

-- Populate mod slugs
WITH numbered_mods AS (
  SELECT id, 
         generate_slug(title) || 
         CASE 
           WHEN ROW_NUMBER() OVER (PARTITION BY generate_slug(title) ORDER BY id) > 1 
           THEN '-' || ROW_NUMBER() OVER (PARTITION BY generate_slug(title) ORDER BY id)::text
           ELSE ''
         END as unique_slug
  FROM mods 
  WHERE slug IS NULL
)
UPDATE mods 
SET slug = numbered_mods.unique_slug
FROM numbered_mods
WHERE mods.id = numbered_mods.id;

-- Populate trail slugs
WITH numbered_trails AS (
  SELECT id, 
         generate_slug(name) || 
         CASE 
           WHEN ROW_NUMBER() OVER (PARTITION BY generate_slug(name) ORDER BY id) > 1 
           THEN '-' || ROW_NUMBER() OVER (PARTITION BY generate_slug(name) ORDER BY id)::text
           ELSE ''
         END as unique_slug
  FROM trails 
  WHERE slug IS NULL
)
UPDATE trails 
SET slug = numbered_trails.unique_slug
FROM numbered_trails
WHERE trails.id = numbered_trails.id;

-- Now create unique indexes
CREATE UNIQUE INDEX idx_vehicles_slug ON vehicles(slug);
CREATE UNIQUE INDEX idx_mods_slug ON mods(slug);
CREATE UNIQUE INDEX idx_trails_slug ON trails(slug);

-- Create triggers to auto-generate slugs for new records
CREATE OR REPLACE FUNCTION auto_generate_vehicle_slug()
RETURNS trigger AS $$
BEGIN
  IF NEW.slug IS NULL OR NEW.slug = '' THEN
    NEW.slug := generate_slug(NEW.brand || '-' || NEW.name || '-' || NEW.year::text);
    
    -- Handle duplicate slugs by appending a number
    WHILE EXISTS (SELECT 1 FROM vehicles WHERE slug = NEW.slug AND id != NEW.id) LOOP
      NEW.slug := NEW.slug || '-' || extract(epoch from now())::int::text;
    END LOOP;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION auto_generate_mod_slug()
RETURNS trigger AS $$
BEGIN
  IF NEW.slug IS NULL OR NEW.slug = '' THEN
    NEW.slug := generate_slug(NEW.title);
    
    -- Handle duplicate slugs by appending a number
    WHILE EXISTS (SELECT 1 FROM mods WHERE slug = NEW.slug AND id != NEW.id) LOOP
      NEW.slug := NEW.slug || '-' || extract(epoch from now())::int::text;
    END LOOP;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION auto_generate_trail_slug()
RETURNS trigger AS $$
BEGIN
  IF NEW.slug IS NULL OR NEW.slug = '' THEN
    NEW.slug := generate_slug(NEW.name);
    
    -- Handle duplicate slugs by appending a number
    WHILE EXISTS (SELECT 1 FROM trails WHERE slug = NEW.slug AND id != NEW.id) LOOP
      NEW.slug := NEW.slug || '-' || extract(epoch from now())::int::text;
    END LOOP;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Create triggers
CREATE TRIGGER vehicle_slug_trigger
  BEFORE INSERT OR UPDATE ON vehicles
  FOR EACH ROW
  EXECUTE FUNCTION auto_generate_vehicle_slug();

CREATE TRIGGER mod_slug_trigger
  BEFORE INSERT OR UPDATE ON mods
  FOR EACH ROW
  EXECUTE FUNCTION auto_generate_mod_slug();

CREATE TRIGGER trail_slug_trigger
  BEFORE INSERT OR UPDATE ON trails
  FOR EACH ROW
  EXECUTE FUNCTION auto_generate_trail_slug();